...
export async function loader({ params, request }: DataFunctionArgs) {
  ...
  return json({
    ...
    brainTreeError,
    error,
    seller, // Retornar o seller no loader
  });
}
...
export async function action({ params, request }: DataFunctionArgs) {
  ...
  const seller = params.seller; // Obtenha o seller dos parâmetros
  ...
  if (typeof paymentMethodCode === 'string') {
    ...
    if (result.addPaymentToOrder.__typename === 'Order') {
      // Redirecionar para a página de confirmação com o seller na URL
      return redirect(
        `/${seller}/checkout/confirmation/${result.addPaymentToOrder.code}`,
      );
    } else {
      throw new Response('Not Found', {
        status: 400,
